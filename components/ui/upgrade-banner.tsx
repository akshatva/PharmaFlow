      ...
      <AnimatePresence>
        ...
        >
          ...
          <div className="relative flex h-[35px] items-center gap-1 rounded-[6px] border border-[#CBE7FF] bg-[#F0F7FF] pl-2.5 pr-1 text-sm">
            <button
              className="rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-[#002359] underline decoration-[#CAE7FF] underline-offset-[5px] outline-none hover:text-[#005FF2] hover:decoration-[#94CCFF]"
            ...
            <span className="text-[0.8125rem] text-[#005FF2]">{description}</span>
            {onClose ? (
              <button
                onClick={onClose}
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-[#005FF2] hover:bg-[#CAE7FF]"
              >
                <X size={16} className="text-[#005FF2]" />
              </button>
            ) : null}
          </div>
        </motion.div>
